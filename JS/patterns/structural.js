...
const cpu = { start:() => 1 }, mem={ load:() => 2 };
const computer={ start:() => [cpu.start(), mem.load()] };
computer.start();



// Bridge - разделяет абстракцию и реализацию, чтобы их можно было менять независимо
...
class Composite { constructor(){this.children=[]} add=c => this.children.push(c) draw => this.children.map(c=>c.draw()).join(',') }
const tree = new Composite(); tree.add(new Leaf()); tree.draw();
